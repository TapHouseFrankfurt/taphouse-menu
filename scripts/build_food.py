...
import html, os, datetime, re, json
...
# ---- schema.org Menu structured data (so AI/Google parse the food as a menu) --
# Built from the same SECTIONS data the page renders, so it never drifts and stays
# current on every rebuild. Linked to the same Restaurant @id the main site emits.
_PRICE_RE = re.compile(r'(\d+(?:[.,]\d+)?)')
def _price_num(s):
    m = _PRICE_RE.search(s or '')
    return m.group(1).replace(',', '.') if m else None
def _diets(pills):
    up = [p.strip().upper() for p in pills]
    if 'VG' in up:  # explicitly vegan
        return ["https://schema.org/VeganDiet", "https://schema.org/VegetarianDiet"]
    if any(p in ('V', 'VG*') for p in up):  # vegetarian (VG* = vegan on request)
        return ["https://schema.org/VegetarianDiet"]
    return []
def _menu_item(d):
    item = {"@type": "MenuItem", "name": d['name']}
    desc = d['en'] or d['de']
    if desc: item['description'] = desc
    pn = _price_num(d['price'])
    if pn: item['offers'] = {"@type": "Offer", "price": pn, "priceCurrency": "EUR"}
    diets = _diets(d['pills'])
    if diets: item['suitableForDiet'] = diets if len(diets) > 1 else diets[0]
    return item
def _build_menu_ld(sections):
    secs = []
    for s in sections:
        items = [_menu_item(d) for b in s['blocks'] for d in b['dishes']]
        if items:
            secs.append({"@type": "MenuSection", "name": s['title'], "hasMenuItem": items})
    graph = {
        "@context": "https://schema.org",
        "@graph": [
            {"@type": "Restaurant", "@id": "https://www.taphousefrankfurt.com/#restaurant",
             "name": "TapHouse Frankfurt", "url": "https://www.taphousefrankfurt.com",
             "servesCuisine": ["Indian", "Craft Beer"],
             "hasMenu": {"@id": "https://menu.taphousefrankfurt.com/food.html#menu"}},
            {"@type": "Menu", "@id": "https://menu.taphousefrankfurt.com/food.html#menu",
             "name": "TapHouse Frankfurt — Food Menu", "inLanguage": "en",
             "url": "https://menu.taphousefrankfurt.com/food.html", "hasMenuSection": secs},
        ],
    }
    return json.dumps(graph, ensure_ascii=False, separators=(',', ':'))
MENU_LD = _build_menu_ld(SECTIONS)
...
<script type="application/ld+json">{MENU_LD}</script>
...
# Write the repo-root food.html (the file build.mjs copies into public/ and deploys),
# and mirror to public/ for local preview parity.
_root_out = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'food.html'))
open(_root_out, 'w').write(HTML)
print("wrote", _root_out, len(HTML), "bytes")
_pub_out = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'public', 'food.html'))
try:
    open(_pub_out, 'w').write(HTML)
    print("wrote", _pub_out, len(HTML), "bytes")
except Exception as _e:
    print("public/ mirror skipped:", _e)
